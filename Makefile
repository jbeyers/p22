...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://p22.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend --build-arg PLONE_VERSION=${PLONE_VERSION} -t jbeyers/p22-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend --build-arg VOLTO_VERSION=${VOLTO_VERSION} -t jbeyers/p22-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name p22-backend-acceptance -d jbeyers/p22-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name p22-frontend-acceptance --link p22-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d jbeyers/p22-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop p22-frontend-acceptance
	@docker stop p22-backend-acceptance
