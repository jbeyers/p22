...
2. Go to your repository at [jbeyers/p22](https://github.com/jbeyers/p22).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `p22.juizi.com` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | p22.juizi.com | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/jbeyers/p22/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/jbeyers/p22/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/jbeyers/p22-backend`) and Frontend (`ghcr.io/jbeyers/p22-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of p22.juizi.com](https://github.com/jbeyers/p22/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/p22.juizi.com.yml` stack.
